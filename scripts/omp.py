...
def BuildOMPDictionary(len_signal):
   #templates = [('flat', 30, 50), ('line', 60, 100), ('trapezoid', 30, 50, 0.15, 0.25)]
   templates = [('flat', 30, 50)]
   #templates = [('trapezoid', 30, 50, 0.15, 0.25)]

   # Count the number of atoms needed
   num_atoms = 0
   for template in templates:
      ...
      for atom_width in range(min_atom_width,max_atom_width+1):
         for start_idx in range(len_signal-atom_width+1):
            ...
            if dict_type == 'flat' or dict_type == 'line':
               num_atoms = num_atoms + 1
            elif dict_type == 'trapezoid':
               ramp_length_min = int(atom_width*template[3])
               ramp_length_max = int(atom_width*template[4])
               for ramp_length in range(ramp_length_min,ramp_length_max+1):
                  num_atoms = num_atoms + 1
            else:
               print 'Error: Unknown template name - FIX ME!'
               pdb.set_trace()
      ...
      for atom_width in range(min_atom_width,max_atom_width+1):
         for start_idx in range(len_signal-atom_width+1):
            ...
            if dict_type == 'flat':
               ...
               omp_dict[start_idx:end_idx,atom_idx] = range(0,end_idx-start_idx)
               atom_idx = atom_idx + 1
            elif dict_type == 'trapezoid':
               ramp_length_min = int(atom_width*template[3])
               ramp_length_max = int(atom_width*template[4])
               for ramp_length in range(ramp_length_min,ramp_length_max+1):
                  omp_dict[start_idx:end_idx,atom_idx] = range(0,ramp_length)+((end_idx-start_idx-2*ramp_length)*[ramp_length-1])+range(ramp_length-1,-1,-1)
                  atom_idx = atom_idx + 1
               #plt.plot(omp_dict[start_idx:end_idx,atom_idx])
               #plt.show()
            else:
...
def ComputeOMP(signal_csv):
   ...
   print 'Building OMP dictionary from templates...'
   omp_dict = BuildOMPDictionary(len(signal))
   print '...Complete! There are %d dictionary elements.'%(omp_dict.shape[0])

   num_atoms_grid_search = range(1,20)#range(15,200,5)
   tolerance = 0.01
   score = []
   mse = []
   for num_atoms_desired in num_atoms_grid_search:
      ...
      score.append(omp.score(omp_dict, signal))
      mse.append(np.mean((omp_signal-signal)**2))
      print 'Finished fitting dictionary with %d atoms'%(num_atoms_desired)

   plt.plot(num_atoms_grid_search, score)
   plt.plot(num_atoms_grid_search, mse)
   plt.legend(['Score', 'MSE'])
   plt.xlabel('Number of atoms used')
