...
def DoOrdinateIntervals(signal_csv, intervals_csv, output_file, comparison_retain_percent=1.0):
   ...
   for i in range(intervals.shape[0]):
      for j in range(intervals.shape[0]):
         ...
         for k in range(j+1, intervals.shape[0]):
            ...
            elif abs(diff_ik - diff_ij) < diff_eps:
   ...
   num_iter = 100
   epsilon = 0.000001
   (embedding_triplets, gamma) = computeEmbedding(n, d, triplets, num_random_restarts=num_restarts, max_num_passes=num_passes, max_iter_GD=num_iter, verbose=True)
